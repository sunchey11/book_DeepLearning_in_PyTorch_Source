...
class SoNN2(SimpleNeuralNetwork):
    """
    预测so表的数据,按天预测
...
sql = """
    select DATE_FORMAT(STR_TO_DATE(so_time, "%%Y-%%m-%%d %%H"),'%%Y-%%m-%%d') so_date,
            so_year,so_quarter,so_month,so_month_day,so_weekday,
            sum(socnt) socnt
    from nn_so_sample wsa
    where warid=%s
        and GOODSID =%s
        and so_year=%s
    group by DATE_FORMAT(STR_TO_DATE(so_time, "%%Y-%%m-%%d %%H"),'%%Y-%%m-%%d'),
            so_year, so_quarter,so_month,so_month_day,so_weekday
    """
rides = pd.read_sql(sql,con=db_conn,params=[1,24229,'2022'],index_col='so_date')
# print(rides)

print("shape:")
print(rides.shape)

dummy_fields = ['so_year','so_quarter','so_month','so_month_day','so_weekday']
fields_to_drop = ['so_year','so_quarter','so_month','so_month_day','so_weekday']
quant_features = ['socnt']
target_fields = ['socnt']
nn:SoNN2 = SoNN2(rides,dummy_fields,fields_to_drop,quant_features,target_fields,360)
nn.prepareData()
nn.train()
nn.showLosses()
nn.showTest()
...
rides2 = pd.read_sql(sql
                    #  +' limit 0,1000'
                     ,con=db_conn,params=[1,24229,'2022'],index_col='so_date')
